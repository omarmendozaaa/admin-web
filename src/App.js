...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${baseURL}/Ticket/last`)
      .then((response) => response.json())
      .then((data) => setIdTick(data));
  }, []);
  ...
  const encodeImageFileAsURL = (element) => {
    ...
    reader.onloadend = function () {
      ...
      fetch(`${PIbaseURL}`, requestOptions)
        .then((response) => response.json())
        .then((data) => {
          setCustomerOut(data.n_personas);
          const requestOptionsOffice = {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              latitude: "-12.094111468918411",
              longitude: "-77.03747002843295",
              cantidadAfuera: data.n_personas,
            }),
          };
          //  'https://bbva20220828000830.azurewebsites.net/api/Office?latitude=asdas&longitude=asdasd&cantidadAfuera=123123' \

          fetch(`${baseURL}/Office?latitude=${-12.094111468918411}&longitude=${-77.03747002843295}&cantidadAfuera=${data.n_personas}`, requestOptionsOffice)
            .then((response) => response.json())
